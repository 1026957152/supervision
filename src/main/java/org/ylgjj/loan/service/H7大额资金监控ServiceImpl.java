...
public class H7大额资金监控ServiceImpl {
    ...
    public Output H_7_1_大额资金监控_监控数据明细查询(String zjbzxbm, String ksrq, String jsrq) {
     ...
     Map<SY_H7大额资金监控, List<FD045_资金划转业务登记文件>> map =  fd045_资金划转业务登记文件s.stream().
        ...
        h7_1大额资金监控_监控数据明细查询.setZjytmc_业务类型("资金调拨");

        if(map.get(SY_H7大额资金监控.je1_金额_500__1000)!= null){
            h7_1大额资金监控_监控数据明细查询.
                    setJe1_金额_500_1000(Double.valueOf(map.get(SY_H7大额资金监控.je1_金额_500__1000).stream().mapToDouble(e->e.getTRANSFERAMT不可为空划转金额()).sum()).toString());
        ...
        if(map.get(SY_H7大额资金监控.je2_金额_1000__1500)!= null){
            h7_1大额资金监控_监控数据明细查询.
                    setJe2_金额_1000_1500(Double.valueOf(map.get(SY_H7大额资金监控.je2_金额_1000__1500).stream().mapToDouble(e->e.getTRANSFERAMT不可为空划转金额()).sum()).toString());
        ...
        if(map.get(SY_H7大额资金监控.je3_金额_1500__2000)!= null){
            h7_1大额资金监控_监控数据明细查询.setJe3_金额_1500_2000(Double.valueOf(map.get(SY_H7大额资金监控.je3_金额_1500__2000).stream().mapToDouble(e->e.getTRANSFERAMT不可为空划转金额()).sum()).toString());
        ...
        if(map.get(SY_H7大额资金监控.je4_金额_2000以上)!= null){
            h7_1大额资金监控_监控数据明细查询.setJe4_金额_2000以上(Double.valueOf(map.get(SY_H7大额资金监控.je4_金额_2000以上).stream().mapToDouble(e->e.getTRANSFERAMT不可为空划转金额()).sum()).toString());
        ...
        output.setData(Arrays.asList(h7_1大额资金监控_监控数据明细查询));
        return output;
    ...
    //    List<FD045_资金划转业务登记文件>  fd045_资金划转业务登记文件s = fd045_资金划转业务登记文件Repository.findByDAMT不可为空借方发生额Between(query.getKsje().doubleValue(),query.getJsje().doubleValue());
        ...
        List map =  fd045_资金划转业务登记文件s.stream().
                map(e->{
                    ...
                    //h7_1大额资金监控_监控数据明细查询.setSpczy_审批人(e.getAGENTOP_可为空_经办柜员());
                    ...
                    h7_1大额资金监控_监控数据明细查询.setSyje_金额_万元(e.getTRANSFERAMT不可为空划转金额()+"");
                    h7_1大额资金监控_监控数据明细查询.setSkzhhm_收款户名(e.getTRSINBANKACC_不可为空_转入银行账户());
                    h7_1大额资金监控_监控数据明细查询.setSkkhyh_收款银行(e.getTRSINBANK_不可为空_转入银行());


                    h7_1大额资金监控_监控数据明细查询.setFkzhhm_付款户名(e.getTRSOUTINST_不可为空_转出机构());
                    h7_1大额资金监控_监控数据明细查询.setFkkhyh_付款银行(e.getTRSOUTBANK_不可为空_转出银行());
                    h7_1大额资金监控_监控数据明细查询.setYhzh_付款账号(e.getTRSOUTBANKACC_不可为空_转出银行账户());

                    //h7_1大额资金监控_监控数据明细查询.setSquname_发起人(e.getAGENTOP_可为空_经办柜员());

                    h7_1大额资金监控_监控数据明细查询.setBz_备注("");
                    h7_1大额资金监控_监控数据明细查询.setZjbzxbm_住建部中心编码("");
