...
public class ZYmygdw每一个单位HistoryerviceImpl {
    ...
    Map<String,DP004_unit_payment_info_单位缴存信息表> dp004_unit_payment_info_单位缴存信息表Map = null;
    ...
    public List<Septet<DP004_unit_payment_info_单位缴存信息表,
        ...
        List<Septet<DP004_unit_payment_info_单位缴存信息表,
                ...
                >> pairs =
                dp004_unit_payment_info_单位缴存信息表s.stream().collect(Collectors.toList()).stream().map(e -> {
                    //TODO        获得某一日的缴存列表;
...
/*            // TODO 按照 贷款年限计算
   ...
   // @PostConstruct
    ...
    public void runTest_____(LocalDate ldt_ksrq, LocalDate js) {
        ...
        DP004_unit_payment_info_单位缴存信息表 dp004_unit_payment_info_单位缴存信息表 = dp004_unit_payment_info单位缴存信息表Repository.findByUnitaccnum单位账号("201000001726");
       // List<DP004_unit_payment_info_单位缴存信息表> dp004_unit_payment_info_单位缴存信息表s = dp004_unit_payment_info单位缴存信息表Repository.findAll();
        ...
        List<DP004_unit_payment_info_单位缴存信息表> dp004_unit_payment_info_单位缴存信息表s = new ArrayList<>();
        dp004_unit_payment_info_单位缴存信息表s.add(dp004_unit_payment_info_单位缴存信息表);
        IntStream.range(0,1).forEach(count-> {

            System.out.println("----------"+count);
            List<DP021_单位缴存登记簿> dp021_单位缴存登记簿s = dp021_单位缴存登记薄Repository.findByUnitaccnum单位账号(dp004_unit_payment_info_单位缴存信息表s.get(count).getUnitaccnum单位账号());
...
/*        private String basenum_缴存基数;
    ...
    public DP008_institution_detail_单位明细账 DP008_institution_detail_单位明细账_flowing(List<DP008_institution_detail_单位明细账> dp008_institution_detail_单位明细账s, LocalDate n) {
        ...
        Map<Boolean,List<DP008_institution_detail_单位明细账>> a = dp008_institution_detail_单位明细账s.stream().collect(Collectors.groupingBy(x->x.getTransdate不可为空交易日期().isBefore(n)));
        ...
        DP008_institution_detail_单位明细账 lastContact = Collections.max(a.get(true),
                Comparator.comparing(DP008_institution_detail_单位明细账::getTransdate不可为空交易日期));

        System.out.println("------- 找的一个最近的登记表啊啊"+lastContact.getTransdate不可为空交易日期());

        System.out.println("------- 单位余额 "+lastContact.getBal_余额());
        return lastContact;
...
*/
    ...
    public void DP021_单位缴存登记簿_flowing_当日动作(List<DP005_work_unit_单位分户账> dp005_work_unit_单位分户账s, LocalDate n) {
        System.out.println("------- 正在处理 的时间"+n);
        // dp004_unit_payment_info_单位缴存信息表s = dp004_unit_payment_info单位缴存信息表Repository.findByUnitaccnum单位账号(dp004_unit_payment_info_单位缴存信息表.getUnitaccnum单位账号());
        Optional<DP005_work_unit_单位分户账> a = dp005_work_unit_单位分户账s.stream().filter(x->x.getOpnaccdate__开户日期().equals(n)).findAny();

        if(a.isPresent()){
            System.out.println("------- 开户"+a.get().getOpnaccdate__开户日期());
        }else{
            System.out.println("------- 没有开户 ");
        }
    }
    public void DP008_institution_detail_单位明细账_flowing_当日动作(List<DP008_institution_detail_单位明细账> dp008_institution_detail_单位明细账s, LocalDate n) {
        ...
        List<DP008_institution_detail_单位明细账> a = dp008_institution_detail_单位明细账s.stream().filter(x->x.getTransdate不可为空交易日期().equals(n)).collect(Collectors.toList());
...
/*        H_227_9160_提取公积金("9160","提取公积金"),
       ...
       // List<DP008_单位明细账> dp021_单位缴存登记簿s = dp008单位明细账Repository.findBySummarycode不可为空摘要代码In(aa);
        ...
        List<DP008_institution_detail_单位明细账> bbb = a.stream().filter(e->{
            return aa.contains(e.getSummarycode());
        ...
        List<DP008_institution_detail_单位明细账> cc = a.stream().filter(e->{
            return aa.contains(e.getSummarycode());
...
/*        private double bfchgunitprop不可为空_变更前单位比例;
    ...
    public void 流水_单位缴存(LocalDate localDate) {
        ...
        if (!initComplte) {
            ...
            cm001_单位基本资料表Map = cm001单位基本资料表Repository.findAll().stream().collect(Collectors.toMap(e -> e.getUnitcustid单位客户号(), a -> a));
            dp005_work_unit_单位分户账Map = dp005_workUnit_单位分户账_repository.findAll().stream().collect(Collectors.toMap(e -> e.getUnitaccnum单位账号(), a -> a));
            dp004_unit_payment_info_单位缴存信息表Map = dp004_unit_payment_info_单位缴存信息表s.stream().collect(Collectors.toMap(e->e.getUnitaccnum单位账号(),a->a));
        ...
        dp005_workUnit_单位分户账_repository.findAll().stream().map(e -> {
            //TODO        获得某一日的缴存列表;
