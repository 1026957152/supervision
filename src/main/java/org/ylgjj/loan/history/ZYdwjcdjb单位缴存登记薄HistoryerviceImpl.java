...
@Service("ZYdwjcdjb单位缴存登记薄HistoryerviceImpl")
public class ZYdwjcdjb单位缴存登记薄HistoryerviceImpl {
...
/*        H_227_9160_提取公积金("9160","提取公积金"),
       ...
       // List<DP008_单位明细账> dp021_单位缴存登记簿s = dp008单位明细账Repository.findBySummarycode不可为空摘要代码In(aa);
        ...
        System.out.println("------- 提取人数"+bbb.stream().mapToInt(e->Long.valueOf(e.getPeoplenum_0_不可为空_人数()).intValue()).sum());
        ...
        System.out.println("------- 异地转入人数"+bbb.stream().mapToInt(e->Long.valueOf(e.getPeoplenum_0_不可为空_人数()).intValue()).sum());
...
/*
    ...
    public void 流水_单位缴存(LocalDate localDate) {
        ...
        List<DP021_单位缴存登记簿> dp021_单位缴存登记簿s = dp021_单位缴存登记薄Repository.findByRegdate不可为空登记日期(localDate);
        List<DW025_公积金提取审核登记表> dw025_公积金提取审核登记表s = dW025__公积金提取审核登记表_Repository.findByTransdate交易日期(localDate);
        List<DP008_单位明细账> DP008_单位明细账s = DP008_单位明细账_repository.findByTransdate不可为空交易日期(localDate);
        ...
        Map<String, List<DP008_单位明细账>> DP008_单位明细账Map
                = DP008_单位明细账s.stream().collect(Collectors.groupingBy(e -> e.getUnitaccnum单位账号()));
