...
public class ZYLoan还款明细HistoryerviceImpl {
    ...
    Map<String,LN003_contract_info_合同信息> ln003_contract_info_合同信息Map = null;
    ...
    public List<Sextet<LN006_RepaymentPlan_贷款分期还款计划,LN005_lone_sub_account_贷款分户信息,LN0014_Trading_house_贷款房屋信息,
                List<LN008_borrower_info_借款人信息>,
                List<LN101_贷款明细账_account>,LN003_contract_info_合同信息>> 历史倒推_某一日的贷款(LocalDate localDate) {
        ...
        List<LN006_RepaymentPlan_贷款分期还款计划> ln006_repaymentPlan_贷款分期还款计划s = ln006_贷款分期还款计划Repository.findByRepaydate1还款日期(localDate);


        //   List<LN101_贷款明细账_account> ln101_贷款明细账_accounts = ln101_贷款明细账_repository.findByTransdate不可为空交易日期Between(ldt_ksrq,ldt_jsrq);


        return ln006_repaymentPlan_贷款分期还款计划s.stream().map(e-> {
            ...
            if(ln101_贷款明细账_account.isPresent()){
                LN003_contract_info_合同信息 ln003_contract_info_合同信息 = ln003_contract_info_合同信息Map.get(e.getLoancontrcode0合同代码());

                return Sextet.with(e,
                        ln005_lone_sub_account_贷款分户信息Map.get(ln003_contract_info_合同信息.getLoancontrnum_借款合同号()),
                        ln0014_trading_house_贷款房屋信息Map.get(ln003_contract_info_合同信息.getLoancontrnum_借款合同号()),
                        ln008_borrower_info_借款人信息Map.get(ln003_contract_info_合同信息.getLoancontrnum_借款合同号()),
                        ln101_贷款明细账_accounts,ln003_contract_info_合同信息);
            }else{
                return null;
            }
    ...
    //也是按照放款的一个流水在进行啊啊。
    //TODO 存储统计信息
    public void saveHistoryOneTime(LocalDate n, List<Sextet<LN006_RepaymentPlan_贷款分期还款计划,
            LN005_lone_sub_account_贷款分户信息,
            LN0014_Trading_house_贷款房屋信息,
            List<LN008_borrower_info_借款人信息>,
            List<LN101_贷款明细账_account>,
            LN003_contract_info_合同信息>> inputs) {
...
/*                StatisticalIndexCodeEnum.S_170_SEQ_回收户数__正常___AND_0302002102
                ...
                loanHistory.setValue贷款金额(uuu.getValue().stream()
                                .filter(z->z.getValue0().getCurseqstate本期状态().equals(E_LN_CurrentSequencePaymentStatusEnum.正常归还.getText()))
...
/*                StatisticalIndexCodeEnum.S_171_SEQ_回收户数__逾期___AND_0302002103
                ...
                loanHistory.setIndex银行名称(uuu.getKey()); // 银行名称
                loanHistory.setValue贷款金额(uuu.getValue().stream()
                        .filter(z->z.getValue0().getCurseqstate本期状态().equals(E_LN_CurrentSequencePaymentStatusEnum.逾期归还.getText()))
                        .mapToDouble(cc->cc.getValue0().getPlanint本期应还利息0().add(cc.getValue0().getPlanprin0本期应还本金()).doubleValue()).sum());  //
                loanHistoryRepository.save(loanHistory);
...
/*                StatisticalIndexCodeEnum.S_172_SEQ_回收户数__提前还本___AND_0302002104
                ...
                loanHistory.setIndex银行名称(uuu.getKey()); // 银行名称
                loanHistory.setValue贷款金额(uuu.getValue().stream()
                        .filter(z->z.getValue0().getCurseqstate本期状态().equals(E_LN_CurrentSequencePaymentStatusEnum.提前部分.getText()))
                        .mapToDouble(cc->cc.getValue0().getPlanint本期应还利息0().add(cc.getValue0().getPlanprin0本期应还本金()).doubleValue()).sum());  //
                loanHistoryRepository.save(loanHistory);
...
/*                StatisticalIndexCodeEnum.S_174_SEQ_回收户数__提前还清___AND_0302002106
                ...
                loanHistory.setIndex银行名称(uuu.getKey()); // 银行名称
                loanHistory.setValue贷款金额(uuu.getValue().stream()
                        .filter(z->z.getValue0().getCurseqstate本期状态().equals(E_LN_CurrentSequencePaymentStatusEnum.提前全部.getText()))
                        .mapToDouble(cc->cc.getValue0().getPlanint本期应还利息0().add(cc.getValue0().getPlanprin0本期应还本金()).doubleValue()).sum());  //
                loanHistoryRepository.save(loanHistory);
...
/*                StatisticalIndexCodeEnum.S_176_SEQ_回收户数__他行回收___AND_0302002108
    ...
    public void saveHistorySpan(LocalDate ldt_ksrq,LocalDate js) {
        ...
        for(int i=0; i<=abs(num); i++) {
            ...
            List<Sextet<LN006_RepaymentPlan_贷款分期还款计划,LN005_lone_sub_account_贷款分户信息,LN0014_Trading_house_贷款房屋信息,
                    List<LN008_borrower_info_借款人信息>,
                    List<LN101_贷款明细账_account>,
                    LN003_contract_info_合同信息>>  aa = 历史倒推_某一日的贷款(n);
...
*/
    ...
    public void 流水_历史还款明细(LocalDate localDate) {
        ...
        ln006_repaymentPlan_贷款分期还款计划s.stream().map(e-> {
