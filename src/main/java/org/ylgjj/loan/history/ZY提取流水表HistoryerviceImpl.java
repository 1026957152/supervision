...
@Service("ZY提取流水表HistoryerviceImpl")
public class ZY提取流水表HistoryerviceImpl {
    ...
    public  List<Octet<DW025_公积金提取审核登记表,
            CM001_单位基本资料表,
            CM002_个人基本资料表,
        ...
        List<DW025_公积金提取审核登记表> dw025_公积金提取审核登记表s1 = dW025__公积金提取审核登记表_Repository.findAll();
              ...
              return  dw025_公积金提取审核登记表s1.stream().collect(Collectors.toList()).stream().map(e->{
            //TODO        获得某一日的缴存列表;


            DP007_individual_sub_account_个人分户账 dp007_individual_sub_account_个人分户账 = dp007_individual_sub_account_个人分户账Map.get(e.getAccnum个人账号());
              ...
              DP005_work_unit_单位分户账 dp005_work_unit_单位分户账 = dp005_work_unit_单位分户账Map.get(e.getUnitaccnum单位账号());
                  ...
                  return Octet.with(e,
                        ...
                        cm002_个人基本资料表Map.get(dp007_individual_sub_account_个人分户账.getCustid_个人客户号()),
                        dp007_individual_sub_account_个人分户账,
    ...
    //TODO 存储统计信息
    public void saveHistoryOneTime(LocalDate n, List<Octet<DW025_公积金提取审核登记表,
            CM001_单位基本资料表,
            CM002_个人基本资料表,
            DP007_individual_sub_account_个人分户账,
            List<DP022_个人缴存登记簿>,
        ...
        inputs.stream().collect(Collectors.groupingBy(e->e.getValue1().getAgentinstcode_经办机构())).entrySet().forEach(eee->{
            ...
            eee.getValue().stream().collect(Collectors.groupingBy(e->{
                return  e.getValue7().stream().filter(i->i.getTransdate不可为空交易日期().isBefore(n)).findFirst().get().getBfchgbasenum_不可为空_变更前基数();

            })).entrySet().forEach(o->{

                // StatisticalIndexCodeEnum.S_20_SEQ_提取金额__收入水平___AND_0301003204

                LoanHistory loanHistory = new LoanHistory(n, StatisticalIndexCodeEnum.S_20_SEQ_提取金额__收入水平___AND_0301003204);
            ...
            eee.getValue().stream().collect(Collectors.groupingBy(e->e.getValue0().getDrawreason_提取原因())).entrySet().forEach(o->{
               ...
               // StatisticalIndexCodeEnum.S_57_SEQ_提取利息__提取原因___AND_0301007711
                ...
                LoanHistory loanHistory = new LoanHistory(n, StatisticalIndexCodeEnum.S_57_SEQ_提取利息__提取原因___AND_0301007711);
                ...
                loanHistory = new LoanHistory(n, StatisticalIndexCodeEnum.S_55_SEQ_提取本金__提取原因___AND_0301007611);
                loanHistory.setIndex提取原因(o.getKey()); // 银行名称
                ...
                loanHistory = new LoanHistory(n,StatisticalIndexCodeEnum.S_36_SEQ_提取人数_AND_0301003901);
            ...
            eee.getValue().stream().collect(Collectors.groupingBy(a->a.getValue1().getUnitkind_单位性质())).entrySet()
                ...
                //StatisticalIndexCodeEnum.S_57_SEQ_提取利息__提取原因___AND_0301007711
                LoanHistory loanHistory = new LoanHistory(n,StatisticalIndexCodeEnum.S_56_SEQ_提取利息_AND_0301007701);
                loanHistory.setIndex机构编码(机构编码); // 机构名称
                ...
                loanHistory = new LoanHistory(n,StatisticalIndexCodeEnum.S_53_SEQ_提取本金_AND_0301007601);
                loanHistory.setIndex机构编码(机构编码); // 机构名称
                loanHistory.setIndex经济类型(uuu.getKey()); // 银行名称
                ...
                loanHistory = new LoanHistory(n,StatisticalIndexCodeEnum.S_19_SEQ_提取人数__非本市缴存职工___AND_0301003203);
                loanHistory.setIndex机构编码(机构编码); // 机构名称
                loanHistory.setIndex经济类型(uuu.getKey()); // 银行名称
                loanHistory.setValue贷款金额(uuu.getValue().stream().mapToDouble(k->k.getValue0().getKEEPBAL_不可为空_提取的存量金额()+k.getValue0().getINCREBAL_不可为空_提取的增量金额()).sum());  //
                loanHistoryRepository.save(loanHistory);
               ...
               // StatisticalIndexCodeEnum.S_23_SEQ_提取金额__非本市缴存职工___AND_0301003207
                loanHistory = new LoanHistory(n,StatisticalIndexCodeEnum.S_23_SEQ_提取金额__非本市缴存职工___AND_0301003207);
                loanHistory.setIndex机构编码(机构编码); // 机构名称
                loanHistory.setIndex经济类型(uuu.getKey()); // 银行名称
                loanHistory.setValue贷款金额(uuu.getValue().stream().mapToDouble(k->k.getValue0().getKEEPBAL_不可为空_提取的存量金额()+k.getValue0().getINCREBAL_不可为空_提取的增量金额()).sum());  //
                loanHistoryRepository.save(loanHistory);
