...
@Service
public class SY__test_______________________176_dkzcspsj_贷款最长审批时间_HistoryerviceImpl {
    ...
    public void process() {
        ...
        List<LN004_合同状态信息> ln004_合同状态信息s = ln004_合同状态信息Repository.findByTransdateBetween(LocalDate.now().minusDays(100),LocalDate.now());
        ...
        ln004_合同状态信息s.stream().collect(Collectors.groupingBy(e->e.getLoancontrcode())).entrySet()
            ...
            RateHistory rateHistory = new RateHistory();
           // E_指标_RATE_SY.SY_1_ljjzzdws_建制总单位数
            rateHistory.setIndexNo(E_指标_RATE_SY.SY_176_dkzcspsj_贷款最长审批时间.get编码());
...
/*        List<Triplet<Long,LocalDate,LocalDate>> triplets =  run统计周期编码(LocalDate.now().minusDays(100),LocalDate.now(),统计周期编码.H__03_每月);
        ...
        for(Triplet<Long,LocalDate,LocalDate> triplet: triplets){
            ...
            num += triplet.getValue0();
            triplet.setAt0(num);

        }*/


   /*     long count = LocalDate.now().minusDays(100).until(LocalDate.now(), ChronoUnit.MONTHS);
        Map<LocalDate,Integer>  mps =  new HashMap();
        Integer num = 0;
        for(long i = count; i >= 0; i--) {
            if(maps.get(LocalDate.now().minusDays(i)) != null){
                num += maps.get(LocalDate.now().minusDays(i)).getValue1();
                mps.put(LocalDate.now().minusDays(i), num);
                System.out.println("------------------------ "+mps.get(LocalDate.now().minusDays(i)));
            }
...
/*            num += maps.get(LocalDate.now().minusDays(i)).getValue1();
        ...
        List<Triplet<Long,LocalDate,LocalDate>> triplets =  run统计周期编码(LocalDate.now().minusDays(100),LocalDate.now(),统计周期编码.H__01_每日);
