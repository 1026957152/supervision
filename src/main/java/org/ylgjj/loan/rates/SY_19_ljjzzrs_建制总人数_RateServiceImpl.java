...
import org.ylgjj.loan.repository.CM002_个人基本资料表Repository;
import org.ylgjj.loan.repository.DP006_个人缴存信息表_Repository;
import org.ylgjj.loan.repository.DP007_个人分户账_Repository;
...
@Service
public class SY_19_ljjzzrs_建制总人数_RateServiceImpl extends RateServiceBaseImpl{


    E_指标_RATE_SY e_指标_rate_sy = E_指标_RATE_SY.SY_19_ljjzzrs_建制总人数;
    @Autowired
    private DP006_个人缴存信息表_Repository dp006_个人缴存信息表_repository;
    @Autowired
    private CM002_个人基本资料表Repository cm002_个人基本资料表Repository;
    @Autowired
    ...
    public void query(H1_2监管主要指标查询_公积金中心主要运行情况查询 h1, String ksrq, String jsrq) {
        ...
        h1.setLjjzzrs_建制总人数_NUMBER_18_0(rateHistory.intValue());
        BigDecimal bigDecimal = BigDecimal.valueOf((rateHistory-rateHistory_环比+0D)/(rateHistory_环比!=0? rateHistory_环比:-1));

        h1.setLjhbjzzrs_环比建制总人数_NUMBER_18_0(bigDecimal.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());

        bigDecimal = BigDecimal.valueOf((rateHistory.intValue()-rateHistory_同比.intValue()+0D)/(rateHistory_同比!=0? rateHistory_同比:-1));

        h1.setLjsnjzzrs_同比建制总人数_NUMBER_18_0(bigDecimal.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
