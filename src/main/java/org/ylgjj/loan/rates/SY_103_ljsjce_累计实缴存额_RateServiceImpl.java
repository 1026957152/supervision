...
public class SY_103_ljsjce_累计实缴存额_RateServiceImpl extends RateServiceBaseImpl{
    ...
    public RateAnalysisStream history() {
        ...
        List<DP021_单位缴存登记簿> ln003_合同信息s = dp021_单位缴存登记薄Repository.findByInaccdate不可为空入账日期BetweenOrderByInaccdate不可为空入账日期Desc(LocalDate.now().minusDays(20000),LocalDate.now());
        System.out.println("-----------------------------"+ ln003_合同信息s.size());




        List<Triplet<LocalDate,Integer,Double>> triplets = ln003_合同信息s.stream().collect(Collectors.groupingBy(e->e.getInaccdate不可为空入账日期()))
                .entrySet()
                .stream()
