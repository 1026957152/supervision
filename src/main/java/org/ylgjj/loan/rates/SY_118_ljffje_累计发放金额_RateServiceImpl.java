...
public class SY_118_ljffje_累计发放金额_RateServiceImpl extends RateServiceBaseImpl {
    ...
    public RateAnalysisStream history(LocalDate beginDate ,LocalDate endDate) {
        ...
        List<Pair<LocalDate,Double>> triplets_stream = ln004_合同状态信息s.stream().collect(Collectors.groupingBy(e->e.getTransdate()))
                .entrySet()
                .stream()
        ...
        for(Pair<LocalDate,Double> triplet: triplets_stream){
