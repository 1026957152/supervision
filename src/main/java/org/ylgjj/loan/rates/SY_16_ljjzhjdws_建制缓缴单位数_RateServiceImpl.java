...
@Service
public class SY_16_ljjzhjdws_建制缓缴单位数_RateServiceImpl extends RateServiceBaseImpl{
 ...
 //   //
    ...
    public RateAnalysisStream history(LocalDate beginDate,LocalDate endDate) {
        ...
        List<DP030_单位缓缴登记簿> ln003_合同信息s = dp030_单位缓缴登记簿_repository
                .findByTransdate不可为空交易日期BetweenOrderByTransdate不可为空交易日期Desc(beginDate.minusDays(1),endDate.plusDays(1));
        System.out.println("-----------------------------"+ ln003_合同信息s.size());


        List<Pair<LocalDate,Long>> sourceList =ln003_合同信息s.stream().collect(Collectors.groupingBy(e->e.getTransdate不可为空交易日期())).entrySet()
                .stream()
...
Triplet<Long,Long,Long> triplet = queryLong期末(e_指标_rate_sy,rateHistories,rateHistories_环比,rateHistories_同比);
        ...
        h1.setLjjzhjdws_建制缓缴单位数_NUMBER_18_0(rateHistory.intValue());
        BigDecimal bigDecimal = BigDecimal.valueOf(rateHistory_环比);

        h1.setLjhbjzhjdws_环比建制缓缴单位数_NUMBER_18_0(bigDecimal.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());

        bigDecimal = BigDecimal.valueOf(rateHistory_同比);

        h1.setLjsnjzhjdws_同比建制缓缴单位数_NUMBER_18_0(bigDecimal.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
