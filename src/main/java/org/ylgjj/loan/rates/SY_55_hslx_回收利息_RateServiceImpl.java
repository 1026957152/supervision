...
public class SY_55_hslx_回收利息_RateServiceImpl extends RateServiceBaseImpl{
    ...
    public RateAnalysisStream history() {
        ...
        List<LN101_贷款明细账> ln003_合同信息s = ln101_贷款明细账_repository.findByTransdate不可为空交易日期BetweenOrderByTransdate不可为空交易日期Desc(LocalDate.now().minusDays(100),LocalDate.now());
   ...
   /*                             if(x.getLoanfundtype不可为空_贷款资金类型().equals(E_LN101_贷款明细账_贷款资金类型.E_))
        ...
        for(Triplet<LocalDate,Double,Double> triplet: sourceList){
            ...
            triplet.setAt2(num);
            triplets.add(Triplet.with(triplet.getValue0(),triplet.getValue1(),triplet.getValue1()));
        }
