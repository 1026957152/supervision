...
@Service
public class S_170_SEQ_回收户数__正常___AND_0302002102_HistoryServiceImpl extends HistoryServiceImpl{
    StatisticalIndexCodeEnum statisticalIndexCodeEnum = StatisticalIndexCodeEnum.S_170_SEQ_回收户数__正常___AND_0302002102;
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<String> list = Arrays.asList(E_LN006_贷款分期还款计划_curseqStatusEnum.正常归还).stream().map(E->E.getText()).collect(Collectors.toList());
        ...
        ln006_贷款分期还款计划s
                .stream()
                .filter(e->e.getCurseqstate本期状态().equals(E_LN006_贷款分期还款计划_curseqStatusEnum.正常归还.getText()))
