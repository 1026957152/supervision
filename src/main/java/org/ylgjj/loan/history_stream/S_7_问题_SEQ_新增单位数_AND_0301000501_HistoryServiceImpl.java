...
@Service
public class S_7_问题_SEQ_新增单位数_AND_0301000501_HistoryServiceImpl extends HistoryServiceImpl{
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<Triplet<Long,LocalDate,LocalDate>> triplets =
                run统计周期编码(beginDateTotal,endDateTotal,StatisticalIndexCodeEnum.S_7_SEQ_销户单位数_AND_0301000601);
        Map<String, CM001_单位基本资料表> finalCm001_单位基本资料表Map = cm001_单位基本资料表Map;
        Map<String, DP005_单位分户账> finalDp005_work_unit_单位分户账Map = dp005_work_unit_单位分户账Map;
        triplets.stream().forEach(t->{
                ...
                return Triplet.with(e,
                    ...
                    .forEach(e -> {
                        ...
                        TargetHistory loanHistory  = new TargetHistory(e.getKey(),
                                StatisticalIndexCodeEnum.S_7_SEQ_销户单位数_AND_0301000601);

                        Long count=e.getValue().stream().map(h->h.getValue2().getClsaccdate销户日期()!= null )
                                .count();

/*                        Long count=e.getValue().stream().map(h->h.getValue6().stream().
                                filter(i->{
