...
@Service
public class S_26_SEQ_实缴人数__月缴存额___AND_0301003210_HistoryServiceImpl extends HistoryServiceImpl{
    StatisticalIndexCodeEnum statisticalIndexCodeEnum = StatisticalIndexCodeEnum.S_26_SEQ_实缴人数__月缴存额___AND_0301003210;
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<DP021_单位缴存登记簿> dp021_单位缴存登记簿s
                = dp021_单位缴存登记薄Repository.findAll();//.collect(Collectors.groupingBy(e->e.getAccnum不可为空个人账号()));
        ...
        List<Triplet<Long,LocalDate,LocalDate>> triplets =
                run统计周期编码(beginDateTotal,endDateTotal,StatisticalIndexCodeEnum.S_26_SEQ_实缴人数__月缴存额___AND_0301003210);
        Map<String, CM001_单位基本资料表> finalCm001_单位基本资料表Map = cm001_单位基本资料表Map;
        Map<String, DP005_单位分户账> finalDp005_work_unit_单位分户账Map = dp005_work_unit_单位分户账Map;
        triplets.stream().forEach(t->{
                ...
                return Triplet.with(e,
                    ...
                    .forEach(e -> {
                        ...
                        TargetHistory loanHistory  = new TargetHistory(e.getKey(),
                                StatisticalIndexCodeEnum.S_26_SEQ_实缴人数__月缴存额___AND_0301003210);

                        ;

/*                        Integer value =e.getValue().stream()
                                .filter(c->c.getValue0().get().plusDays(1).isAfter(beginDate))
