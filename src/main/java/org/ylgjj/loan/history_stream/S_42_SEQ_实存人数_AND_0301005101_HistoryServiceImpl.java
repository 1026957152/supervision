...
@Service
public class S_42_SEQ_实存人数_AND_0301005101_HistoryServiceImpl extends HistoryServiceImpl{
    StatisticalIndexCodeEnum statisticalIndexCodeEnum = StatisticalIndexCodeEnum.S_42_SEQ_实存人数_AND_0301005101;
    @Autowired
    ...
    public void 流水_单位缴存spanTimeSpan(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<DP009_个人明细账> dp009_个人明细账s =   dp009_个人明细账_repository.findByTransdate不可为空交易日期Between(beginDateTotal,endDateTotal);
        ...
        triplets.stream().forEach(t->{
            ...
            dp009_个人明细账s
                ...
                eee.getValue().stream().collect(Collectors.groupingBy(e -> e.getValue1().getUnitkind_单位性质())).entrySet().forEach(o -> {
                    ...
                    Integer value =  o.getValue().stream().mapToInt(x->{
                        if(x.getValue0().getBal_不可为空_余额()> 0 &&  x.getValue0().getBal_不可为空_余额() - x.getValue0().getAmt_不可为空_发生额() == 0)
                            return +1;  //开户了， 之前是空的，现在是满的，
                        if(x.getValue0().getBal_不可为空_余额() - x.getValue0().getAmt_不可为空_发生额() > 0 && x.getValue0().getBal_不可为空_余额() == 0)
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<Triplet<Long,LocalDate,LocalDate>> triplets =
                run统计周期编码(beginDateTotal,endDateTotal,StatisticalIndexCodeEnum.S_40_SEQ_外部转入人数_AND_0301004101);
                ...
                return Triplet.with(e,
                    ...
                    .forEach(e -> {
                        ...
                        TargetHistory loanHistory  = new TargetHistory(e.getKey(),
                                StatisticalIndexCodeEnum.S_40_SEQ_外部转入人数_AND_0301004101);
