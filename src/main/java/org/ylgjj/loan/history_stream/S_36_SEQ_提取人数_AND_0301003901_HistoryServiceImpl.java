...
@Service
public class S_36_SEQ_提取人数_AND_0301003901_HistoryServiceImpl extends HistoryServiceImpl{
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<String> dp = dw025_公积金提取审核登记表s1.stream().map(e->e.getUnitaccnum单位账号()).distinct().collect(Collectors.toList());
        ...
        List<Triplet<Long,LocalDate,LocalDate>> triplets =
                run统计周期编码(beginDateTotal,endDateTotal,StatisticalIndexCodeEnum.S_36_SEQ_提取人数_AND_0301003901);
        Map<String, CM001_单位基本资料表> finalCm001_单位基本资料表Map = cm001_单位基本资料表Map;
        Map<String, DP005_单位分户账> finalDp005_work_unit_单位分户账Map = dp005_work_unit_单位分户账Map;
        triplets.stream().forEach(t->{
