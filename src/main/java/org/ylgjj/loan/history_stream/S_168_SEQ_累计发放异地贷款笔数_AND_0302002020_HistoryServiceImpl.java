...
import org.ylgjj.loan.enumT.E_LN003_合同信息_异地标志;
import org.ylgjj.loan.enumT.E_LN003_合同信息_结清标志;
...
@Service
public class S_168_SEQ_累计发放异地贷款笔数_AND_0302002020_HistoryServiceImpl extends HistoryServiceImpl{
    StatisticalIndexCodeEnum statisticalIndexCodeEnum = StatisticalIndexCodeEnum.S_168_SEQ_累计发放异地贷款笔数_AND_0302002020;
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        //List<String> list = Arrays.asList(E_LN006_贷款分期还款计划_curseqStatusEnum.提前全部).stream().map(E->E.getText()).collect(Collectors.toList());
        ...
        ln003_合同信息s
                .stream()
            //    .filter(e->e.getClearstate不可为空结清标志().equals(E_LN003_合同信息_结清标志.E_1_正常还清.getText()))
                .filter(e->e.getOthplacflag不可为空_异地标志().equals(E_LN003_合同信息_异地标志.E_1_是.getText()))
                ...
                .forEach(f->{
                    ...
                    TargetHistory loanHistory  = new TargetHistory(f.getKey(),
                            statisticalIndexCodeEnum);


                    Long value= count_逾期归还;
