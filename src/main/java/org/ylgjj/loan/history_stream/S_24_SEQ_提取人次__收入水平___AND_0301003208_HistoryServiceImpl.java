...
@Service
public class S_24_SEQ_提取人次__收入水平___AND_0301003208_HistoryServiceImpl extends HistoryServiceImpl{
    StatisticalIndexCodeEnum statisticalIndexCodeEnum = StatisticalIndexCodeEnum.S_24_SEQ_提取人次__收入水平___AND_0301003208;
    ...
    public void 流水_单位缴存spanTimeSpan(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<DW025_公积金提取审核登记表> dw025_公积金提取审核登记表s = dw025_公积金提取审核登记表_repository.findByTransdate交易日期Between(beginDateTotal,endDateTotal);
        ...
        triplets.stream().forEach(t->{
            ...
            dw025_公积金提取审核登记表s
                ...
                eee.getValue().stream().collect(Collectors.groupingBy(e -> e.getValue1().getUnitkind_单位性质())).entrySet().forEach(o -> {
                    ...
                    StreamHistory loanHistory  = new StreamHistory(beginDate,endDate,StatisticalIndexCodeEnum.S_1_SEQ_暂存款笔数_AND_0301000101);
                    ...
                    loanHistory.setDeltaLongValue(o.getValue().stream()
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<Triplet<Long,LocalDate,LocalDate>> triplets =
                run统计周期编码(beginDateTotal,endDateTotal,StatisticalIndexCodeEnum.S_24_SEQ_提取人次__收入水平___AND_0301003208);
        Map<String, CM001_单位基本资料表> finalCm001_单位基本资料表Map = cm001_单位基本资料表Map;
        Map<String, DP005_单位分户账> finalDp005_work_unit_单位分户账Map = dp005_work_unit_单位分户账Map;
        triplets.stream().forEach(t->{
                ...
                return Triplet.with(e,
                    ...
                    .forEach(e -> {
                        ...
                        TargetHistory loanHistory  = new TargetHistory(e.getKey(),
                                StatisticalIndexCodeEnum.S_24_SEQ_提取人次__收入水平___AND_0301003208);


                        Long value= e.getValue().stream().count();
