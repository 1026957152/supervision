...
@Service
public class S_9_SEQ_缓缴单位数_AND_0301000801_HistoryServiceImpl extends HistoryServiceImpl{
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<DP030_单位缓缴登记簿> dp004_单位缴存信息表s = dp030_单位缓缴登记簿_repository.findAll();


        System.out.println("beginDate----------"+ beginDateTotal+"----------endDate----"+endDateTotal);

        List<String> dp = dp004_单位缴存信息表s.stream().map(e->e.getUnitaccnum_单位账号()).distinct().collect(Collectors.toList());
        ...
        List<Triplet<Long,LocalDate,LocalDate>> triplets =
                run统计周期编码(beginDateTotal,endDateTotal,StatisticalIndexCodeEnum.S_9_SEQ_缓缴单位数_AND_0301000801);
        Map<String, CM001_单位基本资料表> finalCm001_单位基本资料表Map = cm001_单位基本资料表Map;
        Map<String, DP005_单位分户账> finalDp005_work_unit_单位分户账Map = dp005_work_unit_单位分户账Map;
        triplets.stream().forEach(t->{
            ...
            dp004_单位缴存信息表s
                ...
                List<DP202_单位缴存变更登记簿> dp202_单位缴存变更登记簿s = dp202_单位缴存变更登记簿Map.get(e.getUnitaccnum_单位账号());
