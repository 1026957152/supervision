...
@Service
public class S_17_SEQ_应缴人数_AND_0301003101_HistoryServiceImpl extends HistoryServiceImpl{
    ...
    public void 流水_单位缴存spanTimeSpan(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<DP204_个人缴存变更登记簿> dp204_个人缴存变更登记簿s
                = dp204_个人缴存变更登记簿_repository.findAll();//.collect(Collectors.groupingBy(e->e.getAccnum不可为空个人账号()));


        System.out.println("beginDate----------"+ beginDateTotal+"----------endDate----"+endDateTotal);

        List<String> dp = dp204_个人缴存变更登记簿s.stream().map(e->e.getUnitaccnum_不可为空_单位账号()).distinct().collect(Collectors.toList());
        ...
        triplets.stream().forEach(t->{
            ...
            dp204_个人缴存变更登记簿s
                ...
                DP005_单位分户账 dp005_单位分户账 = finalDp005_work_unit_单位分户账Map.get(e.getUnitaccnum_不可为空_单位账号());
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
      ...
      //  List<DP202_单位缴存变更登记簿> dp202_单位缴存变更登记簿s = dp202_单位缴存变更登记簿_repository.findAll();
        ...
        Map<String, List<DP202_单位缴存变更登记簿>> dp202_单位缴存变更登记簿Map = dp202_单位缴存变更登记簿Map(dp);
        List<Triplet<Long,LocalDate,LocalDate>> triplets =
                run统计周期编码(beginDateTotal,endDateTotal,StatisticalIndexCodeEnum.S_17_SEQ_应缴人数_AND_0301003101);
        Map<String, CM001_单位基本资料表> finalCm001_单位基本资料表Map = cm001_单位基本资料表Map;
        Map<String, DP005_单位分户账> finalDp005_work_unit_单位分户账Map = dp005_work_unit_单位分户账Map;
        triplets.stream().forEach(t->{
                ...
                return Triplet.with(e,
                    ...
                    .forEach(e -> {
                        ...
                        TargetHistory loanHistory  = new TargetHistory(e.getKey(),
                                StatisticalIndexCodeEnum.S_17_SEQ_应缴人数_AND_0301003101);

                        ;

                        Integer value =e.getValue().stream()

                                .filter(c->c.getValue0().getTransdate不可为空交易日期().plusDays(1).isAfter(beginDate))
                                .mapToInt(x->{
                                    if(x.getValue0().getChgtype_不可为空_变更类型().equals(H_DP204_个人缴存变更登记簿_变更类型.E_4_封存.getText()))
                                        return +1;
                                    if(x.getValue0().getChgtype_不可为空_变更类型().equals(H_DP204_个人缴存变更登记簿_变更类型.E_2_启封.getText()))
                                        return -1;
                                    return 0;
                                }).sum();
