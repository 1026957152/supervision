...
@Service
public class S_5_SEQ_实存单位数_AND_0301000401_HistoryServiceImpl extends HistoryServiceImpl{
    StatisticalIndexCodeEnum statisticalIndexCodeEnum = StatisticalIndexCodeEnum.S_4_SEQ_未缴单位数_AND_0301000301;
    ...
    public void 流水_单位缴存spanTimeSpan(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<DP008_单位明细账> dp008_单位明细账s = dp008__单位明细账_repository.findByTransdate不可为空交易日期BetweenOrderByTransdate不可为空交易日期Desc(beginDateTotal.minusDays(1),endDateTotal.plusDays(1));
      ...
      //  List<DP021_单位缴存登记簿> dp021_单位缴存登记簿s = dp021_单位缴存登记薄Repository.findByRegdate不可为空登记日期Between(beginDateTotal.minusDays(1),endDateTotal.plusDays(1));
        ...
        List<String> dp = dp008_单位明细账s.stream().map(e->e.getUnitaccnum不可为空单位账号()).distinct().collect(Collectors.toList());
        ...
        triplets.stream().forEach(t->{
            ...
            dp008_单位明细账s
                ...
                dimension1.getValue().stream().collect(Collectors.groupingBy(e -> e.getValue1().getUnitkind_单位性质())).entrySet().forEach(dimension2 -> {
                    ...
                    Integer value= dimension2.getValue().stream().collect(Collectors.groupingBy(x->x.getValue2().getUnitaccnum单位账号())).entrySet().stream()
                            .mapToInt(p-> {
    ...
    public void targetHistory(LocalDate beginDateTotal, LocalDate endDateTotal) {
        ...
        List<DP008_单位明细账> dp008_单位明细账s = dp008__单位明细账_repository.findByTransdate不可为空交易日期BetweenOrderByTransdate不可为空交易日期Desc(beginDateTotal,endDateTotal);
            ...
            dp008_单位明细账s
                ...
                return Triplet.with(e,
                    ...
                    .forEach(e -> {
                        ...
                        TargetHistory loanHistory  = new TargetHistory(e.getKey(),StatisticalIndexCodeEnum.S_5_SEQ_实存单位数_AND_0301000401);


                        Integer count= e.getValue().stream().collect(Collectors.groupingBy(x->x.getValue2().getUnitaccnum单位账号())).entrySet().stream()
                                .mapToInt(p-> {
