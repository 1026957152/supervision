...
@Service
public class S_114_SEQ_回收额__他行回收___AND_0302000208_HistoryServiceImpl extends HistoryServiceImpl{
    StatisticalIndexCodeEnum statisticalIndexCodeEnum = StatisticalIndexCodeEnum.S_114_SEQ_回收额__他行回收___AND_0302000208;
            ...
            ln006__贷款分期还款计划s
                ...
                dimension1.getValue().stream().collect(Collectors.groupingBy(e -> {
                    ...
                    Double value =dimension2.getValue().stream()
                            ...
                            .filter(z->{
                                return true;//z.getValue1().stream().filter(g->g.getAgentbankcode不可为空_经办银行().equals(z.getValue5().getAgentbankcode_经办银行())).count()>0;
                            })
